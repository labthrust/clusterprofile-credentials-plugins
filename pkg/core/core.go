...
type Provider interface {
	// Name returns provider name for logs
	Name() string
	// GetTokenJSON returns an ExecCredential JSON based on the provided ExecCredential input
...
func Run(p Provider) {
	...
	if plugin == "" {
		fmt.Fprintln(os.Stderr, "[core] provider Name() returned empty string; this is not allowed")
		os.Exit(1)
	}
...
// BuildExecCredentialJSON is a helper to construct a minimal ExecCredential JSON
